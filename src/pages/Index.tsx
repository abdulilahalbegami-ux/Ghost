...
const Index = () => {
  ...
  const getSmartResponse = (category: string, userText: string): string => {
    ...
    if (
      ...
      if (lower.includes("python") || lower.includes("sort") || lower.includes("array")) {
        return applyFlavor(
          `Here is an efficient quicksort implementation in Python (O(N log N)):\n\n` +
    ...
    if (category === "video_generation") {
      return applyFlavor(`Generated HD 1080p motion sequence for: "${userText}". Rendered with 60fps frame interpolation.`);
    }

    if (category === "image_generation") {
      return applyFlavor(`Synthesized high-fidelity visual asset for prompt: "${userText}". Rendered at 1024x1024 resolution.`);
    }

    if (category === "doc_analysis") {
      return applyFlavor(`Parsed document "${selectedDoc?.name || "file"}":\n1. Extracted document structure and metadata.\n2. Identified key concepts, dates, and action items.\n3. Saved summary to Nuvio Memory Core.`);
    }

    if (category === "image_analysis") {
      return applyFlavor("Image computer vision matrix scan complete. Foreground objects, color histogram, and OCR text extracted.");
    }

    if (category === "pizza") {
      return applyFlavor("Domino's Pepperoni Pizza is available for $12.99 with coupon code '50OFF'. Fast delivery estimated in 20-30 minutes.");
    }

    if (category === "haircut") {
      return applyFlavor("Haircut appointment with stylist Alex at Downtown Barbers is available tomorrow at 5:30 PM.");
    }

    if (category === "dubai") {
      return applyFlavor("4-Day Dubai trip breakdown under $500:\n• Round-trip Flight: $240 (FlyDubai)\n• Accommodation: $135 (3 nights at Dubai Marina Hostel)\n• Local Transport & Food: $95");
    }

    if (category === "messages") {
      return applyFlavor("Unread channels scan:\n• 3 Slack messages\n• 2 Emails\nMost urgent: Request from Sarah regarding Project Nuvio deliverable.");
    }

    // Natural default AI response
    return applyFlavor(
      `${userText.charAt(0).toUpperCase() + userText.slice(1)} is an important topic. Based on ${aiModel}, the optimal approach focuses on systematic execution, clear principles, and reliable performance.`
    );
  };
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-black text-zinc-900 dark:text-white font-sans flex flex-col md:flex-row transition-colors duration-200">
      ...
      {showAuthModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex<dyad-write path="src/pages/Index.tsx" description="Writing complete src/pages/Index.tsx file to ensure no cutoffs or syntax errors.">
"use client";
          ...
          <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-white/10 rounded-2xl p-6 max-w-sm w-full space-y-4 relative">
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <button
                ...
                disabled={socialLoadingProvider !== null}
                className="flex items-center justify-center py-2 bg-zinc-100 dark:bg-white/5 hover:bg-zinc-200 dark:hover:bg-white/10 border border-zinc-200<dyad-write path="src/pages/Index.tsx" description="Writing complete Index page with full auth modal markup and conversational response logic.">
"use client";
